...
export default function HomeScreen() {
  ...
  // Navigation function
  // Updated startNavigation to use enqueueInitialNavigation
      ...
      if (isNavigationRequest) {
        for (const keyword of navigationKeywords) {
          if (transcript.toLowerCase().includes(keyword)) {
            extractedDestination = transcript.toLowerCase().split(keyword)[1]?.trim() || "";
            break;
          }
        }
        startNavigation(extractedDestination);
      } else {
        const response = await axios.post("http://localhost:8000/command", {
          userInput: transcript,
          sessionId: "unique-session-id",
        });
        console.log("Backend response:", response.data); // Log backend result as in original
  ...
  return (
    <View style={styles.container}>
      <TouchableOpacity
        ...
        onPress={async () => {
          // Test a voice - change the identifier to test different voices
          speakWithElevenLabs("This is a test of the text to speech functionality using this voice");
        }}
      >
        <Text style={styles.buttonText2}>Test Voice</Text>
      </TouchableOpacity>
