...
const AISettingsSlider: React.FC<AISettingsSliderProps> = ({
  ...
  const fetchAISettings = async () => {
    try {
      setIsLoading(true);
      const response = await axios.get(`http://10.0.0.215:8000/api/ai-settings/${userId}`);
  ...
  const updateTemperature = async (value: number) => {
    try {
      ...
      await axios.post('http://10.0.0.215:8000/changeTemperature', {
  ...
  const updateHumorLevel = async (value: number) => {
    try {
      ...
      const response = await axios.post('http://10.0.0.215:8000/setHumorLevel', {
  ...
  const updateFrequency = async (value: number) => {
    try {
      ...
      await axios.post('http://10.0.0.215:8000/setFrequency', {
